...
var React = _interopRequireWildcard(__webpack_require__(4410));
...
var _scrollHandler = __webpack_require__(1432);
var _react = __webpack_require__(4410);
...
var _react = _interopRequireDefault(__webpack_require__(4410));
...
const trimmedPathname=stripPrefix(pathname,decodeURIComponent("/public"))// Remove any hashfragment
...
if(this.staticQueryDb[staticQueryHash]){const jsonPayload=this.staticQueryDb[staticQueryHash];return{staticQueryHash,jsonPayload};}return this.memoizedGet(`${"/public"}/page-data/sq/d/${staticQueryHash}.json`).then(req=>{const jsonPayload=JSON.parse(req.responseText);return{staticQueryHash,jsonPayload};}).catch(()=>{throw new Error(`We couldn't load "${"/public"}/page-data/sq/d/${staticQueryHash}.json"`);});})).then(staticQueryResults=>{const staticQueryResultsMap={};staticQueryResults.forEach(({staticQueryHash,jsonPayload})=>{staticQueryResultsMap[staticQueryHash]=jsonPayload;this.staticQueryDb[staticQueryHash]=jsonPayload;});return staticQueryResultsMap;});return Promise.all([componentChunkPromises,staticQueryBatchPromise]).then(([pageResources,staticQueryResults])=>{let payload;if(pageResources){payload={...pageResources,staticQueryResults};finalResult.payload=payload;emitter.emit(`onPostLoadPageResources`,{page:payload,pageResources:payload});}this.pageDb.set(pagePath,finalResult);if(finalResult.error){return{error:finalResult.error,status:finalResult.status};}return payload;})// when static-query fail to load we throw a better error
...
const React=__webpack_require__(4410);const{grabMatchParams}=__webpack_require__(9755);const{StaticQueryContext}=__webpack_require__(7076);const{headExportValidator,filterHeadProps,isElementType,isValidNodeName,warnForInvalidTag}=__webpack_require__(9706);const{ServerLocation,Router}=__webpack_require__(7896);const{renderToString}=__webpack_require__(9698);const{parse}=__webpack_require__(698);const styleToOjbect=__webpack_require__(7848);function applyHtmlAndBodyAttributesSSR(htmlAndBodyAttributes,{setHtmlAttributes,setBodyAttributes}){if(!htmlAndBodyAttributes)return;const{html,body}=htmlAndBodyAttributes;setHtmlAttributes(html);setBodyAttributes(body);}function getValidHeadNodesAndAttributesSSR(rootNode,htmlAndBodyAttributes={html:{},body:{}}){const seenIds=new Map();const validHeadNodes=[];// Filter out non-element nodes before looping since we don't care about them
 ...
 * LICENSE file in the root directory of this source tree.
 */
var aa=__webpack_require__(6464),e=__webpack_require__(4410),k=null,l=0,m=!0;function p(a,b){a=a.write(b);m=m&&a}
...
"use strict";
module.exports = require("/Users/jonathanlane/Documents/Development/Jono Lane/.cache/ssr-builtin-trackers/fs");
...
"use strict";
module.exports = require("/Users/jonathanlane/Documents/Development/Jono Lane/node_modules/react-dom/server.js");
...
"use strict";
module.exports = require("/Users/jonathanlane/Documents/Development/Jono Lane/node_modules/react/index.js");
...
const React=__webpack_require__(4410);const path=__webpack_require__(1423);const{renderToStaticMarkup,renderToPipeableStream}=__webpack_require__(9698);const{ServerLocation,Router,isRedirect}=__webpack_require__(7896);const merge=__webpack_require__(9996);const{StaticQueryContext}=__webpack_require__(7076);const fs=__webpack_require__(7942);const{WritableAsPromise}=__webpack_require__(5897);const{RouteAnnouncerProps}=__webpack_require__(236);const{apiRunner,apiRunnerAsync}=__webpack_require__(4067);const asyncRequires=__webpack_require__(5418);const{version:gatsbyVersion}=__webpack_require__(1741);const{grabMatchParams}=__webpack_require__(9755);const{headHandlerForSSR}=__webpack_require__(923);const{SlicesResultsContext,SlicesContext,SlicesMapContext,SlicesPropsContext}=__webpack_require__(8995);const{ServerSliceRenderer}=__webpack_require__(7529);// we want to force posix-style joins, so Windows doesn't produce backslashes for urls
...
await apiRunnerAsync(`replaceRenderer`,{bodyComponent,replaceBodyHTMLString,setHeadComponents,setHtmlAttributes,setBodyAttributes,setPreBodyComponents,setPostBodyComponents,setBodyProps,pathname:pagePath,pathPrefix:"/public"});// If no one stepped up, we'll handle it.
...
if(!isRedirect(e))throw e;}}apiRunner(`onRenderBody`,{setHeadComponents,setHtmlAttributes,setBodyAttributes,setPreBodyComponents,setPostBodyComponents,setBodyProps,pathname:pagePath,loadPageDataSync,bodyHtml,scripts,styles,pathPrefix:"/public"});// we want to run Head after onRenderBody, so Html and Body attributes
